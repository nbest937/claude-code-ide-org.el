<!-- Generated by claude-org-setup from the claude-code-ide-org plugin.
     ...
     skills/org/references/org-event-queue.md — re-run claude-org-setup to refresh.
...
**With time tracking off, only the state half of that is live.** The
clock tools still queue and nothing consumes it (`org-time-tracking.md`).
The queue exists for org's state-change logging, which needs a genuinely
