...
| `skills/brainstorming/` | the design-dialogue skill, ported from superpowers (see its `README.md` and `.upstream`); `.claude/skills/brainstorming` is a symlink into it |
...
| `plans/` | frozen archive of the Plan Mode plan files older headings link; plans now live in each heading's `:PLAN:` drawer |
