...
### 4.4 Use dedicated checker subagents for judgment-level curation

For the class of task that genuinely needs LLM judgment rather than a mechanical check — deduplicating newly noticed ideas against existing headings, assessing slice/story fit, spotting undeclared dependencies, nominating NEXT candidates, and especially detecting **twins** (the failure mode the project's own conventions already say "is caught by review, never by the composer") — the concrete mechanism is a dedicated subagent, invoked for that one check, with a narrow, saturated context: the relevant convention section plus the specific query result, not the full rule stack. A subagent built this way will outperform the same judgment made mid-task by the orchestrator juggling everything else, for the same reason a rule buried in a large standing context loses to one it was actually asked to apply.

This is a different use of subagents than "more workers running the same task with the same full context" — that version doesn't help, since it just multiplies the number of agents carrying the same crowded rule set. The value here comes specifically from *narrowing*: each checker subagent gets only what it needs to answer one question, which is what makes it reliable where the orchestrator, mid-task, is not.

Two constraints on these checker subagents, to stay consistent with the project's existing "queue, don't apply" discipline:
