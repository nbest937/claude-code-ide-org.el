...
- **In a repo using the claude-code-ide-org event queue** (this project;
  ...
  **only where time tracking is switched on** — and *nothing edits the
  ...
  `org-time-tracking.md` is promoted only where setup was told the
  feature is on, and its absence means it is off. Never hand-write CLOCK lines, keywords, or `- State` notes
